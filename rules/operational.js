...
module.exports = {
  ...
  show_command: $ => seq(
    optional(choice($._prompt_exec, $._prompt_config)),
    choice(
      $.show_version_block,
  ...
  show_version_block: $ => seq(
    ...
    repeat(choice(
      ...
      $.config_register,
      $._newline,
      prec.dynamic(100, alias($._line_content, $.unknown_line))
    ))
  ),
  ...
  show_inventory_block: $ => seq(
    ...
    repeat(choice($.inventory_name_line, $.inventory_pid_line, $._newline, prec.dynamic(100, alias($._line_content, $.unknown_line))))
  ),
  ...
  show_ip_int_brief_block: $ => seq(
    ...
    repeat(choice($.ip_int_brief_header, $.ip_int_brief_entry, $._newline, prec.dynamic(100, alias($._line_content, $.unknown_line))))
  ),
  ...
  show_ip_interface_block: $ => seq(
    ...
    optional(/[ \t]+/),
    $._newline,
    repeat(choice($.interface_status_entry, $._newline, prec.dynamic(100, alias($._line_content, $.unknown_line))))
  ),
  ...
  show_environment_block: $ => seq(
    ...
    repeat(choice($.environment_info, $._newline, prec.dynamic(100, alias($._line_content, $.unknown_line))))
  ),
  ...
  show_ip_arp_block: $ => seq(
    ...
    repeat(choice($.arp_header, $.arp_entry, $._newline, prec.dynamic(100, alias($._line_content, $.unknown_line))))
  ),
  ...
  show_ip_route_block: $ => seq(
    ...
    optional(/[ \t]+/),
    $._newline,
    repeat(choice($._routing_generic_line, $.subnet_header, $.routing_entry, $._newline, prec.dynamic(100, alias($._line_content, $.unknown_line))))
  ),
  ...
  show_ip_bgp_summary_block: $ => seq(
    ...
    repeat(choice($.bgp_summary_header, $.bgp_summary_entry, $._newline, prec.dynamic(100, alias($._line_content, $.unknown_line))))
  ),
  ...
  show_ip_ospf_neighbor_block: $ => seq(
    ...
    repeat(choice($.ospf_neighbor_header, $.ospf_neighbor_entry, $._dashed_line, $._newline, prec.dynamic(100, alias($._line_content, $.unknown_line))))
  ),
  ...
  show_crypto_ipsec_sa_block: $ => seq(
    ...
    repeat(choice($.crypto_ipsec_sa_entry, $._newline))
  ),
  crypto_ipsec_sa_entry: $ => seq(
    token(prec(120, /[Ii]nterface\s*:/i)), field('interface', choice($.interface_name, $.word)), $._newline,
    repeat(choice($.crypto_map_line, $.crypto_ident_line, $.crypto_peer_line, $.crypto_pkts_line, $._newline))
  ),
  crypto_map_line: $ => seq(token(prec(120, /[Cc]rypto\s+map\s+tag\s*:/i)), field('tag', $.word), ',', token(prec(120, /seq\s+num\s*:/i)), field('seq', $.number), ',', token(prec(120, /local\s+addr\s*:/i)), field('local_addr', $.ipv4_address), $._newline),
  crypto_ident_line: $ => seq(field('side', choice(token(prec(120, /local/i)), token(prec(120, /remote/i)))), token(prec(120, /ident/i)), '(', token(prec(120, /addr\/mask\/prot\/port/i)), '):', '(', field('address', $.ipv4_address), '/', field('mask', choice($.ipv4_address, $.number)), '/', field('protocol', $.number), '/', field('port', $.number), ')', $._newline),
  crypto_peer_line: $ => seq(token(prec(120, /current_peer\s*:/i)), field('peer', $.ipv4_address), optional(seq(token(prec(120, /port\s*:/i)), field('port', $.number))), $._newline),
  crypto_pkts_line: $ => seq(choice(seq(token(prec(120, /#pkts\s+encaps\s*:/i)), field('encaps', $.number)), seq(token(prec(120, /#pkts\s+decaps\s*:/i)), field('decaps', $.number))), repeat(seq(token(prec(130, ',')), choice($.word, $.number, $.punctuation, /#[a-z]+/i, /:[a-z]+/i))), $._newline),

  show_crypto_ikev1_sa_block: $ => seq(token(prec(120, seq(/[Ss]how\s+[Cc]rypto\s+[Ii]kev1\s+[Ss][Aa]/i, optional(/[Dd]etail/i), optional(/[ \t]+/)))), $._newline, repeat(choice($.ikev1_sa_header, $.ikev1_sa_entry, $._newline))),
  ikev1_sa_header: $ => prec(200, seq(token(prec(120, /IKEv1\s+SAs:/i)), $._newline, optional($._dashed_line))),
  ikev1_sa_entry: $ => seq(field('connection_id', $.number), field('ip_address', $.ipv4_address), field('state', $.word), field('role', choice(token(prec(120, /INITIATOR/i)), token(prec(120, /RESPONDER/i)))), $._newline),

  show_failover_block: $ => seq(token(prec(200, seq(/[Ss]how\s+[Ff]ailover/i, optional(/[ \t]+/)))), $._newline, repeat(choice($.failover_line, $._newline))),
  failover_line: $ => seq(token(prec(150, /[Ff]ailover/i)), choice(seq($._whitespace, field('status', alias(choice(/[Oo]n/i, /[Oo]ff/i), $.word))), seq($._whitespace, /[Uu]nit/i, $._whitespace, field('role', $.word))), $._newline),
  failover_interface_line: $ => seq(field('interface_label', alias(repeat1($.word), $.text)), ':', field('interface', choice($.interface_name, $.word)), field('status', alias(repeat1($.word), $.text)), $._newline),

  show_vpn_sessiondb_block: $ => seq(token(prec(120, seq(/[Ss]how\s+[Vv]pn-sessiondb/i, optional(choice(/[Aa]nyconnect/i, /[Ll]2[Ll]/i)), optional(/[ \t]+/)))), $._newline, repeat(choice($.vpn_session_summary, $.vpn_session_entry, $._newline))),
  vpn_session_summary: $ => seq(token(prec(120, /Total\s+active\s+and\s+inactive\s*:/i)), field('total', $.number), $._newline),
  vpn_session_entry: $ => seq(token(prec(120, /Index\s*:/i)), field('index', $.number), ',', token(prec(120, /Username\s*:/i)), field('username', $.word), ',', token(prec(120, /Public\s+IP\s*:/i)), field('public_ip', $.ipv4_address), $._newline),

  show_bgp_neighbors_block: $ => seq(token(prec(120, seq(/[Ss]how\s+[Bb][Gg][Pp]\s+[Nn]eighbors/i, optional(/[ \t]+/)))), $._newline, repeat(choice($.bgp_neighbor_summary_line, $._newline))),
  bgp_neighbor_summary_line: $ => seq(token(prec(120, /BGP\s+neighbor\s+is/i)), field('neighbor', choice($.ipv4_address, $.ipv6_address)), ',', token(prec(120, /remote\s+AS/i)), field('remote_as', $.number), ',', token(prec(120, /local\s+AS/i)), field('local_as', $.number), $._newline),

  show_mpls_ldp_neighbor_block: $ => seq(token(prec(120, seq(/[Ss]how\s+[Mm][Pp][Ll][Ss]\s+[Ll][Dd][Pp]\s+[Nn]eighbor/i, optional(choice(/[Bb]rief/i, /[Dd]etail/i)), optional(/[ \t]+/)))), $._newline, repeat(choice($.mpls_ldp_neighbor_line, $._newline))),
  mpls_ldp_neighbor_line: $ => seq(token(prec(120, /Peer\s+LDP\s+ID:/i)), field('peer_id', $.ipv4_address), ':', field('label_space', $.number), ';', token(prec(120, /Local\s+LDP\s+ID:/i)), field('local_id', $.ipv4_address), ':', field('local_label_space', $.number), $._newline),

  show_vpc_block: $ => seq(token(prec(120, seq(/[Ss]how\s+vpc/i, optional(/[ \t]+/)))), $._newline, repeat(choice($.vpc_summary_line, $.vpc_entry, $._dashed_line, $._newline))),
  vpc_summary_line: $ => seq(token(prec(120, /vPC\s+domain\s+id\s*:/i)), field('domain_id', $.number), optional($._whitespace), $._newline),
  vpc_entry: $ => seq(field('vpc_id', $.number), field('port', choice($.interface_name, $.word)), field('status', $.word), field('consistency', $.word), optional($._whitespace), $._newline),

  show_fex_block: $ => seq(token(prec(120, seq(/[Ss]how\s+fex/i, optional(/[ \t]+/)))), $._newline, repeat(choice($.fex_entry, $._newline))),
  fex_entry: $ => seq(field('fex_id', $.number), field('description', $.word), field('state', $.word), field('model', $.word), field('serial', $.word), $._newline),

  show_etherchannel_summary_block: $ => seq(token(prec(120, seq(/[Ss]how\s+[Ee]therchannel\s+[Ss]ummary/i, optional(/[ \t]+/)))), $._newline, repeat(choice($.etherchannel_header, $.etherchannel_entry, $._dashed_line, $._newline))),
  etherchannel_header: $ => prec(200, seq(token(prec(120, /Group/i)), /Port-channel/i, /Protocol/i, /Ports/i, $._newline)),
  etherchannel_entry: $ => seq(field('group', $.number), field('bundle_name', choice($.interface_name, $.word)), field('bundle_status', $.word), field('bundle_protocol', $.word), repeat1(seq(field('member_interface', choice($.interface_name, $.word)), '(', field('member_status', $.word), ')')), $._newline),

  show_spanning_tree_block: $ => seq(token(prec(120, seq(/[Ss]how\s+[Ss]panning-tree/i, optional(/[ \t]+/)))), optional($.word), $._newline, repeat(choice($.spanning_tree_vlan_header, $.spanning_tree_entry, $._dashed_line, $._newline))),
  spanning_tree_vlan_header: $ => seq(token(prec(120, /VLAN/i)), field('vlan_id', $.number), optional(alias($.text_line, $.text)), $._newline),
  spanning_tree_entry: $ => seq(field('interface', choice($.interface_name, $.word)), field('role', $.word), field('status', $.word), field('cost', $.number), field('port_priority', $.number), '.', field('port_id', $.number), field('type', optional($.word)), $._newline),

  show_spanning_tree_block: $ => seq(token(prec(120, seq(/[Ss]how\s+[Ss]panning-tree/i, optional(/[ \t]+/)))), optional($.word), $._newline, repeat(choice($.spanning_tree_vlan_header, $.spanning_tree_entry, $._dashed_line, $._newline))),
  spanning_tree_vlan_header: $ => seq(token(prec(120, /VLAN/i)), field('vlan_id', $.number), optional(alias($.text_line, $.text)), $._newline),
  spanning_tree_entry: $ => seq(field('interface', choice($.interface_name, $.word)), field('role', $.word), field('status', $.word), field('cost', $.number), field('port_priority', $.number), '.', field('port_id', $.number), field('type', optional($.word)), $._newline),

  show_standby_block: $ => seq(token(prec(120, seq(/[Ss]how\s+[Ss]tandby/i, optional(/[ \t]+/)))), optional(token(prec(120, /[Bb]rief/i))), $._newline, repeat(choice($.standby_header, $.standby_entry, $._newline))),
  ...
  standby_entry: $ => seq(field('interface', choice($.interface_name, $.word)), field('group', $.number), field('priority', $.number), field('preempt', $.word), field('state', $.word), field('active_router', choice($.ipv4_address, $.word)), field('standby_router', choice($.ipv4_address, $.word)), field('virtual_ip', $.ipv4_address), $._newline)
};
